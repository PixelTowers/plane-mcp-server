...
const PLANE_SERVER_URL = process.env.PLANE_SERVER_URL;
...
async function callPlaneAPI(
  ...
  let baseUrl = 'https://api.plane.so/api/v1/workspaces/';
  if (PLANE_SERVER_URL) {
    baseUrl = `${PLANE_SERVER_URL}/api/v1/workspaces/${PLANE_WORKSPACE_SLUG}`;
  }
